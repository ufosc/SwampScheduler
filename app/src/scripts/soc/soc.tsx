...
export abstract class SOC_Generic {
     ...
     * @returns The UID.
     */
    static formUID(courseInd: number, sectionInd: number | undefined = undefined) {
    ...
    searchCourses(searchBy: SearchBy, phrase: string): Course[] {
        ...
        if (!phrase) // Empty search phrase should not return all courses
            return [];
        ...
        if (searchBy === SearchBy.COURSE_CODE) {
            ...
            return this.courses.filter(c => c.name.toUpperCase().includes(upperPhrase))
        }
...
export class SOC_API extends SOC_Generic {
    ...
    fetchSearchCourses(searchBy: SearchBy, phrase: string, cont: AbortController): CancellablePromise<Course[]> {
    ...
    private async fetchCourses(searchBy: SearchBy, phrase: string, controller: AbortController, lcn = 0): Promise<void> {
        if (!phrase)
        ...
        console.log(`Fetching by ${searchBy} for "${phrase}"${lcn > 0 ? ` @ #${lcn}` : ''}`);

        const searchURL = 'https://one.uf.edu/apix/soc/schedule'
        ...
        return fetch(searchURL, {signal: controller.signal})
            ...
            .then(async j => {
                ...
                if (RETRIEVEDROWS == 50)
                    await this.fetchCourses(searchBy, phrase, controller, LASTCONTROLNUMBER);
            }).then(() => {
