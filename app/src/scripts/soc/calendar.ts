...
export function handleExportScheduleClick(schedule: Schedule) {
    try {
        ...
        for (const section of schedule) {
            ...
            for (const day of API_Days) {
                ...
                for (const meeting of section.meetings[day]) {
                    const firstStartDate = getDate(
                            section.startDate,
                            meeting.timeBegin,
                        ),
                        firstEndDate = getDate(
                            section.startDate,
                            meeting.timeEnd,
                        );
                    ...
                    const startDayOfWeek =
                        (firstStartDate.weekday() -
                            moment().day("Monday").weekday() +
                            7) %
                        7;
        ...
        const file = new File([icalContent], "swampschedule.ics", {
            type: "text/calendar",
        });
